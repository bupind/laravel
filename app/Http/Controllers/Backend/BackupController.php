...
class BackupController extends Controller
{
        ...
        try {
            ...
            if($exitCode !== 0) {
                ...
                return redirect()->back()->with(
                    'error',
                    $this->flashMessage(
                        $message !== '' ? 'notifications.backup.failed_with_reason' : 'notifications.common.failed',
                        ['message' => $message]
                    )
                );
            }
    ...
    public function delete(string $file): RedirectResponse
    {
        ...
        $disk->delete($path);
        return redirect()->back()->with('success', $this->flashMessage('notifications.common.deleted'));
    }
}
